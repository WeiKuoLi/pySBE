...
from sbe.semiconductors import BandStructure2D, BandStructure3D, SimpleSemiconductor
...
def main2D():
    ...
    semiconductor = SimpleSemiconductor()
    bs = BandStructure2D(material=semiconductor, edges_c=np.array([0.0]), edges_v=np.array([0.0]))
    energy, ans, _ = absorption(bs, **params)
    energy, ans1, fig = absorption(bs, cc=False, **params)

    plt.plot((energy - semiconductor.Eg/e)/semiconductor.E_0*e, ans / np.max(ans), 'k')
    plt.plot((energy - semiconductor.Eg/e)/semiconductor.E_0*e, ans1 / np.max(ans), 'k--')
    plt.xlim([-20, 20])
    # plt.xlim([1.4, 1.7])
    # plt.ylim([0, 1.4])
...
def main3D():
    ...
    semiconductor = SimpleSemiconductor()
    bs = BandStructure3D(material=semiconductor, edges_c=np.array([0.0]), edges_v=np.array([0.0]))
